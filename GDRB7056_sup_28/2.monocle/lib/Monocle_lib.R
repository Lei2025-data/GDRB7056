...
Initial <- function( object, cells.use = NULL, assay.use = "RNA" ) {
		if ( is.null(object) ){
		...
		if (("negbinomial" == mnc_obj@expressionFamily@vfamily) || ("negbinomial.size" == mnc_obj@expressionFamily@vfamily)) {	
				...
				genes <- select@assays[[assay.use]]@var.features
		}
...
DoMonocle <- function( mnc_obj, ordering_genes = NULL, max_components = 2, maxIter = 20, ncenter = NULL, tol = 0.001, sigma = 0.001, lambda = NULL, lambda.fold = NULL, param.gamma = 10 ) {
#		if ( is.null(lambda) ){
#				if ( is.null(lambda.fold) ) {
#						lambda.fold <- 2;
#				}
#				lambda <- ncol(mnc_obj) * lambda.fold
#		}
		...
		if (ncol(mnc_obj) >= 100) {
				mnc_obj <- reduceDimension(mnc_obj, method = 'DDRTree', verbose = F, scaling = T, norm_method = norm_method )
		}else{
				mnc_obj <- reduceDimension(mnc_obj, method = 'DDRTree', verbose = F, scaling = T, norm_method = norm_method, ncenter = ncol(mnc_obj) - 1 )
		}
#		mnc_obj <- reduceDimension(mnc_obj, method = 'DDRTree', verbose = F, scaling = T, max_components = max_components, norm_method = 'log', maxIter = maxIter, ncenter = ncenter, tol = tol, sigma = sigma, lambda = lambda, param.gamma = param.gamma )
...
FindBranchName <- function( cds, branch_point ) {
		if(cds@dim_reduce_type == "DDRTree") {
		...
		root_cell      <- paste0( "Y_", closest_vertex[root_cell,1] )
	
		path_to_ancestor <- shortest_paths(pr_graph_cell_proj_mst, branch_point, root_cell)
		...
		for (backbone_nei in neighbors( pr_graph_cell_proj_mst, branch_point )$name ) {
				...
				if (root_cell %in% descendents == FALSE){
						path_to_point <- unique( c(path_to_ancestor, branch_point, descendents ) )
						if (cds@dim_reduce_type == "DDRTree"){
								cells_in_path <- row.names(closest_vertex)[which(V(pr_graph_cell_proj_mst)[closest_vertex]$name %in% path_to_point)]
						}else{
								cells_in_path <- path_to_point
						}
						cells_in_path <- intersect(cells_in_path, colnames(cds))
						sub_state <- pData(cds)[cells_in_path,"State", drop = F]
						state_name <- as.character(unique(dplyr::arrange(sub_state, State)$State))
						BranchName[[backbone_nei]] <- paste0( "State ", paste( state_name, collapse = "," ) )
				}
		}
		return(BranchName)
}
...
ChangeBranchPointState <- function(cds) {
		...
		for ( branch_point in cds@auxOrderingData[[cds@dim_reduce_type]]$branch_points ) {
				...
				path_to_ancestor <- names(unlist(path_to_ancestor$vpath))
				next_cells_to_root <- rownames(subset(closest_vertex, name == path_to_ancestor[2]))
...
MonocleObject <- function(parameter = list(), cls_col_name = "seurat_clusters",
				mnc_obj = parameter$mnc_obj, obj_file = parameter$obj_file,
				cell_use = parameter$cell_use, cell.set = parameter$cell.set,
...
RunMonocle <- function(mnc_obj, parameter = list(), feature_gene_use = parameter$feature_gene_use, ...){
				genes.use <- if ( ! is.null(feature_gene_use) && ! file.exists(feature_gene_use) ) readLines(feature_gene_use) else NULL
				mnc_obj <- DoMonocle( mnc_obj, ordering_genes = genes.use, ... )
				pData(mnc_obj) <- droplevels(pData(mnc_obj))
				return(mnc_obj)
}

RenameState <- function(mnc_obj, parameter = list(), rename_state = parameter$rename_state, root_state = parameter$root_state){
		...
		if ( !is.null(root_state) )
				if ( pData(mnc_obj)$State[which.min(pData(mnc_obj)$Pseudotime)] != root_state )
						mnc_obj <- orderCells( mnc_obj, root_state = root_state )
		return(mnc_obj)
}
...
Diff.State <- function(mnc_obj, thres = 1e-7, cores = 1, use.q = TRUE, diff_state_res = NULL){
		...
		write.table(sig_gene_names, file = "Diff.state_heatmap.list.xls", quote = F, sep = "\t", row.names = F)
...
Diff.Pseudotime <- function(mnc_obj, thres = 1e-7, cores = 1, use.q = TRUE, diff_Pseudotime_res = NULL){
		...
		write.table(sig_gene_names, file = "Diff.pseudotime_heatmap.list.xls", quote = F, sep = "\t", row.names = F)
...
Diff.Branch <- function(mnc_obj, thres = 1e-7, cores = 1, use.q = TRUE, BEAM_res = NULL) {
		...
		for ( i in seq(branch_point) ) {
				...
				if ( length(BEAM_res) < i ) 
						BEAM_res[[i]] <- BEAM(mnc_obj, branch_point = i, cores = cores, branch_labels = branch_name )
				saveRDS( BEAM_res[[i]], file = paste0( "BEAM.", i, ".rds") )
				...
				p5 <- plot_genes_branched_heatmap(mnc_obj[sig_gene_names,], branch_point = i, num_clusters = 5, cores = cores, show_rownames = T, return_heatmap=T, branch_labels = branch_name )
				...
				write.table(sig_gene_names, file = paste0( "Branch.", branch_point_index[branch_point[i]], ".genes_heatmap.list.xls"), quote = F, sep = "\t", row.names = F)

				BEAM_sig <- BEAM_res[[i]][BEAM_res[[i]]$gene_short_name %in% rownames(p5$annotation_row),]
				BEAM_sig <- data.frame( GeneID = rownames(BEAM_sig), Gene_cluster = p5$annotation_row[as.character(BEAM_sig$gene_short_name), "Cluster"], P_value = BEAM_sig$pval, FDR = BEAM_sig$qval, row.names = rownames(BEAM_sig) )
				...
				total_BEAM_sig[[i]] <- data.frame(branch_node = branch_point_index[branch_point[i]],
												  ...
												  BEAM_sig)
#				BEAM_res[[i]] <- BEAM_res[[i]][sig_gene_names,]
		}
...
PlotPseudotimeHeatmap <- function(mnc_obj, features, parameter, cores = 1, outfile = NULL, ...){
		is.legend <- if(parameter$legend$position == "none") FALSE else TRUE
		title <- if (is.null(parameter$labs$title)) NA else parameter$labs$title
		nstates <- length(levels(pData(mnc_obj)$State))

		if ( length(unlist(parameter$legend$color)) < 3 ) {
				hmcols <- monocle:::blue2green2red(100)
		}else{
				hmcols <- colorRampPalette(c(parameter$legend$color$low, parameter$legend$color$mid, parameter$legend$color$high))(100)
		}
		ph <- plot_pseudotime_heatmap(mnc_obj[features,], num_clusters = nstates, cores = cores, show_rownames = T, return_heatmap = T,
						cluster_rows = parameter$hclust$row, hmcols = hmcols, scale_max = parameter$data$scale.range, scale_min = parameter$data$scale.range * -1)
...
#					width = min(15, max(7, parameter$font$size + 1)),
#					height = min(30, max(7, length(features) * 0.1)))
		}
		if ( ! is.null(parameter$labs$xlab) || ! is.null(parameter$labs$ylab) ) {
				require(grid)
				setHook("grid.newpage", function() pushViewport(viewport(x=1,y=1,width=0.9, height=0.9, name="vp", just=c("right","top"))), action="prepend")
		}
		if ( ! is.null(ph$annotation_row) ) colnames(ph$annotation_row) <- "Gene_cluster"
		pheatmap::pheatmap(ph$heatmap_matrix[,],						
				...
				cluster_rows = parameter$hclust$row, cluster_cols = FALSE,
...
#				filename = outfile, width = 7, height = NA,
		...
		if ( ! is.null(parameter$labs$xlab) || ! is.null(parameter$labs$ylab) ) {
				...
				grid.text(parameter$labs$xlab, y=-0.05, gp=gpar(fontsize=parameter$font$size))
				grid.text(parameter$labs$ylab, x=-0.05, rot=90, gp=gpar(fontsize=parameter$font$size))
		}
...
PlotBranchHeatmap <- function(mnc_obj, features, parameter, cores = 1, outfile = NULL, branch_point = NULL, ...){
		...
		branch_name <- FindBranchName(mnc_obj, branch_point_name)
		...
		ph <- plot_genes_branched_heatmap(mnc_obj[features,], branch_point = branch_point,
						...
						branch_labels = branch_name, cluster_rows = parameter$hclust$row, hmcols = hmcols,
						scale_max = parameter$data$scale.range,
						scale_min = parameter$data$scale.range * -1 )
		height <- grid::convertHeight(sum(ph$ph_res$gtable$heights), "inches", valueOnly = T)
		width  <- grid::convertWidth(sum(ph$ph_res$gtable$widths), "inches", valueOnly = T)
		while(!is.null(dev.list())) { 
				dev.off()
