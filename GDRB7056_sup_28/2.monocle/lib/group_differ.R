FindGroupIndex <- function(object, parameter = list(), slot = "counts", assay = NULL, sample_col = "orig.ident",
...
FindGroupMarkers <- function (object, group.data, contrast = NULL,
		...
		min.cells.group = 3, pseudocount.use = 1, is.bulk = TRUE, ...) {
...
DoFindGroupMarkers <- function(object, group.data, parameter = list(),
				...
				min.pct         = IfNull(parameter$FindMarkers$min_pct, 0.1),
				pseudocount.use = IfNull(parameter$FindMarkers$pseudocount.use, 1),
				...){
...
WriteDifferMarker <- function(marker, add_name = NULL, object = NULL) {
		...
		all.list <- do.call(rbind, lapply(names(marker), function(i) cbind(contrast = i, marker[[i]][,c("cluster", "gene")])) )
...
CalAvgByGrpInCls <- function(object, group.data, group_names,
				...
				is.expm1 = ifelse(slot=="data", TRUE, FALSE), is.bulk = TRUE,
		...
		all_mean_exp <- NULL
		clusters <- if ( is.bulk ) c("bulk", levels(Idents(object))) else levels(Idents(object))
...
#						Gene_name <- FindFeaturesName(object, rownames(mean_exp), "name")
						...
						WriteTable(out_mean_exp, file = paste(outpref, i, "xls", sep = "."))
				}
...
get_gene_cell_info <- function(object, sample = "orig.ident", cluster = "seurat_clusters", assay = "RNA", outfile = NULL) {
		info      <- object@meta.data %>% group_by(sample = orig.ident, cluster = seurat_clusters, group) %>% summarise(total = n()) %>% mutate(mg = paste(sample, cluster, sep = "_"))
		...
		data <- f(sum_count, "sum") %>%
				...
				arrange(GeneID) %>%
				select(GeneID, GeneName, cluster, sample, group, total, sum, sum_normal, num)
