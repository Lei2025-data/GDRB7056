DoNormalization <- function(object, parameter = list(), assay = "RNA", is_SCTransform = FALSE, vars.regress = NULL, is.check = TRUE, nfeatures = 2000, normalization.method = "LogNormalize", sct.method = "poisson", scale.only.var.genes = FALSE, ...){
		...
		if ( is_SCTransform ) {
				...
				object <- SCTransform(object, assay = assay, vars.to.regress = object@misc$vars.regress,
				                      verbose = FALSE, min_cells = 1, method = sct.method, return.only.var.genes = scale.only.var.genes)
		}else{
				### Normalize Data
				...
				message( "-->ScaleData<--" )
				features <- if ( scale.only.var.genes ) NULL else rownames(object)
				object <- ScaleData(object = object, vars.to.regress = object@misc$vars.regress, features = features)
		}
		return(object)
}
