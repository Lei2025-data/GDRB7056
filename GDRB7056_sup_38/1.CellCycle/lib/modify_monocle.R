    ...
    cores = 1) 
{
	...
	if ( nrow(heatmap_matrix) < 2 ) cluster_rows <- FALSE #
    ...
    cores = 1, ...) 
{
    ...
    BranchAB_exprs <- genSmoothCurves(new_cds[, ], cores = cores, 
        trend_formula = trend_formula, relative_expr = T, new_data = rbind(newdataA, 
            newdataB))
    BranchA_exprs <- BranchAB_exprs[, 1:100, drop = F] #
    BranchB_exprs <- BranchAB_exprs[, 101:200, drop = F] #
    ...
    heatmap_matrix <- cbind(BranchA_exprs[, (col_gap_ind - 1):1, drop = F], #
        BranchB_exprs)
    ...
    heatmap_matrix <- heatmap_matrix[is.finite(heatmap_matrix[, 
        1]) & is.finite(heatmap_matrix[, col_gap_ind]), , drop = F] #
    row_dist <- as.dist((1 - cor(Matrix::t(heatmap_matrix)))/2)
    row_dist[is.na(row_dist)] <- 1
    ...
    horizontal_jitter = NULL, ggplot_scale = scale_y_log10()) 
{
	...
	min_expr2 <- max(min_expr, 10^-300) ## 
    cds_exprs$expression[cds_exprs$expression < min_expr2] <- min_expr2    ##
    cds_exprs$expectation[cds_exprs$expectation < min_expr2] <- min_expr2  ##
	if ( anyNA(cds_exprs$expectation) ) {
			ggplot_scale <- NULL
	}
...
plot_genes_branched_pseudotime <- function (cds, branch_states = NULL, branch_point = 1, branch_labels = NULL, 
    ...
    label_by_short_name = TRUE, relative_expr = TRUE, ...) 
{
    ...
    cds_exprs$full_model_expectation[cds_exprs$full_model_expectation < 
        min_expr] <- min_expr
	cds_exprs$full_model_expectation[cds_exprs$full_model_expectation < 10^-300] <- 10^-300 ## ggplot2 cannot handle it
    if (!is.null(reducedModelFormulaStr)) {
...
environment(plot_genes_branched_pseudotime) <- asNamespace("monocle")
assignInNamespace("plot_genes_branched_pseudotime", plot_genes_branched_pseudotime, ns = "monocle")
