...
DoFindAllMarkers <- function(object, parameter = list(), group.by = "seurat_clusters",
				min.pct = eval(parse(text = IfNull(parameter$FindMarkers$min_pct, 0.25))),
				logfc.threshold = eval(parse(text = IfNull(parameter$FindMarkers$logfc, 0.25))),
				return.thresh = eval(parse(text = IfNull(parameter$FindMarkers$pvalue, 0.01))),
				pseudocount.use = eval(parse(text = IfNull(parameter$FindMarkers$pseudocount.use, 0))),
				only.pos = eval(parse(text = IfNull(parameter$FindMarkers$only.pos, TRUE))),
				base = exp(1),
				...)
{
		...
		if ( packageVersion("Seurat") < as.numeric_version("4.0.0") ) {
				...
				object.markers <- FindAllMarkers(object = object, only.pos = only.pos,
						min.pct = min.pct, logfc.threshold = logfc.threshold,
						return.thresh = return.thresh, pseudocount.use = pseudocount.use, base = base, ...)
		}
...
ListMarker <- function(object, object.markers, outfile = "DeGene.list.xls", is.return = FALSE, is.fast = FALSE, group.by = "seurat_clusters",
		...
		marker_list <- object.markers %>% left_join(y = Targets_mean) %>% left_join(y = Others_mean) %>% 
				...
				select(!! Targets_name := cluster, "Gene ID" = gene, "Gene Name" = name,
						!! Targets_pct_name := pct.1, !! Others_pct_name := pct.2,
...
FindTopMarker <- function(object.markers, top_num = 20, object = NULL, outfile = "Top.avg_exp.xls.tmp"){
		...
		top <- object.markers %>% group_by( cluster ) %>%
				arrange(desc(!! logfc_name), p_val, p_val_adj, .by_group = TRUE) %>% filter(1:n() <= top_num)
		if ( ! is.null(outfile) ) {
